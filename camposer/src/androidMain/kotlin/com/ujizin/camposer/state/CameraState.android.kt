...
public actual class CameraState internal constructor(
  ...
  public actual var imageCaptureStrategy: ImageCaptureStrategy by distinctConfig(
    ...
    val mode =
      when {
        value == ImageCaptureStrategy.MinLatency && !isZSLSupported -> value.fallback
        else -> value.mode
      }
  ...
  public actual var isTorchEnabled: Boolean by distinctConfig(
    ...
    check = {
      check(
        !it || cameraInfo.isTorchAvailable,
  ...
  public actual var videoStabilizationMode: VideoStabilizationMode by distinctConfig(
    check = {
      throw NotImplementedError("Video stabilization mode is not implemented in Android yet")
    },
  ...
  private fun getUseCases(mode: CaptureMode = captureMode) =
    when {

...
import com.ujizin.camposer.extensions.systemVersion
...
internal object CameraFormatUtils {
    ...
    fun getPhotoFormats(
        formats: List<AVCaptureDeviceFormat>,
    ): List<CameraData> = formats.map { format ->
        memScoped {
            val fallback = format.highResolutionStillImageDimensions.placeTo(this).pointed

            val dimen = if (UIDevice.systemVersion >= 16.0) {
                format.supportedMaxPhotoDimensions.filterIsInstance<CMVideoDimensions>()
